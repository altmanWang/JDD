...
def get_order_feature(order, MONTH, NUM, uid):
    ...
    price_sum_mean["price_sum_mean"] = price_sum_mean["price_sum_mean"].apply(lambda x: (x - price_sum_mean["price_sum_mean"].min()) / (price_sum_mean["price_sum_mean"].max() - price_sum_mean["price_sum_mean"].min()))
    #####
    ...
    price_mean["price_mean"] = price_mean["price_mean"].apply(lambda x: (x - price_mean["price_mean"].min()) / (price_mean["price_mean"].max() - price_mean["price_mean"].min()))
    #每月最低/最高/平均购买次数
    ...
    average_price_each = history.groupby(["uid"]).agg({"price":"mean"}).rename(columns={"price":"average_price_each"}).reset_index()
    ...
    std_price_each = history.groupby(["uid"]).agg({"price": "std"}).rename(
        columns={"price": "std_price_each"}).reset_index()
...
def get_loan_feature(loan, MONTH, NUM, uid):
    ...
    loan_sum_mean["loan_sum_mean"] = loan_sum_mean["loan_sum_mean"].apply(lambda x: (x - loan_sum_mean["loan_sum_mean"].min()) / (loan_sum_mean["loan_sum_mean"].max() - loan_sum_mean["loan_sum_mean"].min()))
    ...
    pay_per_month_mean["pay_per_month_mean"] = pay_per_month_mean["pay_per_month_mean"].apply(lambda x: (x - pay_per_month_mean["pay_per_month_mean"].min()) / (pay_per_month_mean["pay_per_month_mean"].max() - pay_per_month_mean["pay_per_month_mean"].min()))
...
def get_loan_limit_ratio(user, loan, MONTH):
    loan["next_month_remain_loan"] = loan.loc[loan["month"]<=MONTH+1].apply(get_remain_loan, axis=1, args=(MONTH+1,))
    loan["next_month_remain_loan"] = loan["next_month_remain_loan"] .fillna(0)
    next_month_remain_loan = pd.DataFrame(loan.loc[loan["month"]<=MONTH]["next_month_remain_loan"].groupby([loan["uid"]]).sum()).reset_index()

    user = pd.DataFrame(user[["uid","limit"]])
    user["limit"] = user["limit"].apply(lambda x : np.round(5**x-1,2 ))

    user = pd.merge(user, next_month_remain_loan, on=["uid"], how="left")
    user["loan_limit_ratio"] = user.apply(lambda x : x["next_month_remain_loan"] / x["limit"], axis =1 )
